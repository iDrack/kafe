...
import 'package:kafe/models/etat_pousse.dart';
    ...
    Widget build(BuildContext context, WidgetRef ref) {
      ...
      useEffect(() {
        ...
        timer = Timer.periodic(const Duration(seconds: 1), (timer) async {
          final newRemainingTime = tempsDePousseEffectif -
              DateTime.now().difference(datePlantation!).inSeconds;
          ...
          if (newRemainingTime > 0) {
            ...
            remainingTime.value = newRemainingTime;
            if (newRemainingTime <= 0 && newRemainingTime > -tempsDePousseEffectif) {
      ...
      void recolter() {
        if (etatPousse.value == EtatPousse.EnCours) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('${etatPousse.value.message}.')),
          );
        } else {
      ...
      String formatTime(int seconds) {
        final minutes = seconds < 0 ? "00" : (seconds ~/ 60).toString().padLeft(2, '0');
        final secs = seconds < 0 ? "00" : (seconds % 60).toString().padLeft(2, '0');
        return "$minutes:$secs";
      }
