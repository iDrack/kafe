...
class CompetitionCard extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    Future<void> collectRewards() async {
      rewards.value = await ref
          .watch(competitionStreamProvider.notifier)
          .claimReward(competition, user!.uuid);
      showDialog(
        context: context,
        builder: (BuildContext context) {
          return RecompenseAlert(
            deevee: rewards.value["deevee"] ?? 0,
            goldenSeed: rewards.value['goldenSeed'] ?? 0,
          );
        },
      );
    }
      ...
      child: Column(
        children: [
          ListTile(
            title: Text("Victoire !"),
            subtitle: Text(
              'Le ${DateFormat("dd/MM/yyyy à HH:mm").format(competition.dateEpreuve)}',
            ),
